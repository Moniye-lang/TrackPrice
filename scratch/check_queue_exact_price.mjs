...
async function checkQueueForExactPrice() {
    try {
        ...
        console.log(`Checking ${pendingUpdates.length} pending updates for exact price matches with Supermart...\n`);

        for (const update of pendingUpdates) {
            const match = scrapedProducts.find(s => s.price === update.price);

            if (match) {
                const product = await Product.findById(update.productId).lean();
                console.log(`EXACT PRICE MATCH FOUND for Update ${update._id}:`);
                console.log(`- Proposed Price: ₦${update.price}`);
                console.log(`- Supermart Match: ${match.name} (₦${match.price})`);
                if (product) {
                    console.log(`- DB Product: ${product.name}`);
                    console.log(`- DB Store: ${product.storeLocation}`);
                }
                console.log('---');
            }
        }
